...
**Opintopolku** (2026-07-21 perusta; laajennettu 2026-08-04–08-06 Taitosolmut/Siltasolmut/Huolilippu/Aikaikkuna/Henkselit-integraatiolla, ja 2026-08-05 taas Hytti/Opintopolku-auditoinnilla + HYTTI_SPEKSI_2026-08-05.md:n osittaisella toteutuksella — tämä osio oli pitkään VANHENTUNUT, ei maininnut mitään 2026-08-04 jälkeen rakennetusta ennen tätä päivitystä.)
- Mitä tekee: Katrin yksityinen opiskelusuunnitelmamoduuli Hytin sisällä. Kurssi → aiheet/**opiskelusolmut** (`opinto_aiheet`, PACER-vaihetila `priming`/`encoding`/`retrieval`/`reference`/`yllapito`, kullakin nyt myös `viimeksi_kosketettu`-aikaleima ja per-otsikko `materiaali`-linkki, ks. sql/110) → deadlinet kurssi- tai aihetasolla → kevyt materiaali-tekstikenttä kurssilla. "Kolmen voiman moottori" (puhdas laskenta, EI älykutsua siinä itsessään) punnitsee deadlinea + PACER-järjestystä + Kuormavahtia JA `taitosolmut`-siltoja, tarjoaa 1-2 päivän askelta ohjaavassa ikkunassa Hytin yläosassa (`#opinto-tanaan-osio`), tallennettu idempotentisti. "Tehty" etenee PACERia ja ajastaa spaced repetitionin (1/3/7/21pv → ylläpitosykli joka PITENEE 60 päivästä 1.5x/kierros, katto 365pv, ks. `yllapidonSeuraavaVali()`). `reference`-vaihe on TIETOISESTI moottorin ulkopuolella (vain käsin asetettava). Kokonaiskartta-näkymä (`opinto-kartta-view`) on puhtaasti luettava. **PACER-ohje** ("Näytä ohje" -nappi Tänään-kortilla, ℹ️-nappi kurssin aihelistalla ja taitosolmun näkymässä) näyttää vaihekohtaisen yleisohjeen + kohteen oman materiaali/linkki-kentän jos asetettu; lisäksi jokaisella Tänään-kortilla on eksplisiittinen "Ehdotus: ✓ Tehty siirtää vaiheeseen X" -kehoterivi (kytkettävissä pois: Asetukset → 🚦 Kuormavahti → `pacer_kehote_paalla`).
- **Taitosolmut/Siltasolmut** (2026-08-04, uudelleenmääritelty 2026-08-05): `taitosolmut` = VAIN silta-käsitteitä jotka toistuvat ≥2 samanaikaisesti aktiivisessa kurssissa, EI koskaan yhden kurssin sisältöä, EI koskaan `kurssi_id`-saraketta (ks. KONSEPTIKIRJA.md 4.11-lisäys ja OSA 1 periaate 14). `taito_kaaret` = suunnatut kaaret solmujen välillä (`tarvitsee`=esitietoportti, `liittyy`=vain kartalle). `taitosolmu_viittaukset` = linkki yksittäiseen `opinto_aiheet`-riviin, kertoo mitä silta yhdistää. "🌉 Etsi sillat" -toiminto (`etsiSiltoja()`) kutsuu `/api/aly`:tä, ehdottaa siltoja AKTIIVISTEN kurssien materiaalista, ihminen hyväksyy rivi kerrallaan ennen tallennusta. Jokaisella taitosolmulla on OMA käsitekartta (kynä+kumi+tekstilaatikot -canvas-editori, `taitosolmut.kasitekartta`/`kasitekartta_tekstit`, EI OCR:ää/älyä piirroksen tulkinnassa). **Tunnettu, korjattu 2026-08-05 -bugi:** moottori (`laskeOpintoPaivanAskeleet`) palautti aiemmin TYHJÄN jos taitosolmut/taito_kaaret/taitosolmu_viittaukset-kyselyt epäonnistuivat (esim. migraatio ajamatta) — EI YHTÄÄN ehdotusta edes pelkille opinto_aiheet-käyttäjille. Nyt vain silta-puolen virhe lokitetaan, aihe-puoli toimii silti.
- **Huolilippu** (2026-08-04, `paivan_huolet`): ennakoiva, kaksisuuntainen (±14pv) kuormamerkintä, OMA mekanismi kuin ristiriitapaketin päällekkäisyysmerkki — nostaa vain kuormatasoa, ei koskaan laske.
- **Aikaikkuna** (2026-08-05, `etsiIkkunaanSopivaAskel`): "minulla on N minuuttia" -kertakysely samalla moottorilla, suodatettuna kestoarviolla (`kesto_<vaihe>_min`-asetukset). Ei tallennu `opinto_paivan_askeleet`-tauluun.
- **Session-loki** (2026-08-05, `opinto_sessiot`): ▶ Aloita/⏸ Lopeta -ajastin per askel, yksi kesken oleva istunto per käyttäjä. Kirjoituspuoli valmis, EI toistaiseksi mitään lukunäkymää (tarkoituksella — kerätään dataa ensin, ei kalibrointialgoritmia vielä).
- **Henkselit-kytkentä** (2026-08-05, ks. Henkselit-osio muualla tässä tiedostossa): oman henkselöinnin ajaksi `laskeOpintoPaivanAskeleet` palauttaa tyhjän JA Hytin omat ankkurit piilotetaan — sama choke point, per-henkilö ohitettavissa asetuksista.
- **Materiaalin syöttöputki — tekstiosa** (2026-08-05, ks. HYTTI_SPEKSI_2026-08-05.md §9): paikallinen heuristiikka tunnistaa Laituriin kirjoitetun/liitetyn kurssimateriaalin (≥3 päivämäärää + riittävä pituus), tarjoaa "Tarkista"-kortin joka `/api/aly`-jäsentää sen kurssiksi+aiheiksi+deadlineiksi esitäytettynä, muokattavana lomakkeena. **Liitetiedostojen (ppt/pdf/kuvat/doc) raahaus/jäsennys EI ole rakennettu** — vaatisi uuden tiedostojen tallennus- ja käsittelyinfrastruktuurin, tietoisesti rajattu pois tästä erästä.
- Tiedostot: `index.html` (`opinto-kurssi-view`, `opinto-kartta-view`, `taitosolmu-view`, `materiaali-jasennys-overlay`, `silta-ehdotus-overlay`), `script.js` (`// === OPINTOPOLKU VAIHE 1 ===`, `// === OPINTOPOLKU VAIHE 2: KOLMEN VOIMAN MOOTTORI ===`, `// === TAITOSOLMUT ===`, `// === KÄSITEKARTTA-EDITORI ===`, `// === MATERIAALIN SYÖTTÖPUTKI — TEKSTIOSA ===`).
- Taulut: `opinto_kurssit`, `opinto_aiheet` (+ `viimeksi_kosketettu`/`materiaali`, sql/110), `opinto_deadlinet`, `opinto_paivan_askeleet`, `taitosolmut`, `taito_kaaret`, `taitosolmu_viittaukset`, `opinto_sessiot`, `paivan_huolet` — kaikki owner_id-RLS samalla kaavalla kuin `hytti_kortit`.
- **Täysi, ajantasainen inventaario + tunnetut aukot:** ks. HYTTI_SPEKSI_2026-08-05.md ja sen taustalla oleva auditointi (muistiinpanot.md 2026-08-05). Erityisesti: taitosolmujen ≥2-kurssi-sääntöä ei valvota tietokantatasolla eikä käsin lisättävässä kaari-editorissa, vain AI-ehdotuksen hyväksyntäsuodattimessa. Speksissä ehdotettu suurempi uudelleenjärjestely (rajaton/panoroitava kanvaasi, kurssikohtainen "Taulu", IA-uudelleenjärjestely kahdeksi "oveksi", Taitokartta-automaatio, opiskelukalenteri) on TIETOISESTI RAKENTAMATTA — Katri rajasi ne pois 2026-08-05 rakennuserästä eksplisiittisesti.
