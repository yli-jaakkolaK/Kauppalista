...
- [ ] **Kalenterisilta (2026-07-18) — EI TESTATTU LAINKAAN, kriittisin osa vaatii oikean laitteen.** Odota (tai luo testiksi) ✨-ehdokas jolla on selkeä aika+aihe, tarkista että "➕ Lisää kalenteriin" -nappi näkyy kortilla. Napauta sitä OIKEALLA iPhonella (ei riitä tietokoneella/Playwrightilla) — tarkista avautuuko iOS:n oma "uusi tapahtuma" -näkymä esitäytettynä oikealla otsikolla/päivällä/ajalla. Jos EI avaudu odotetusti (näyttää raakaa tekstiä, lataa tiedoston, tms.), kerro tarkalleen mitä tapahtui — `avaaKalenteriSilta()`-funktio saatetaan joutua vaihtamaan blob:/webcal:-pohjaiseksi.
...
**TEKNINEN EPÄVARMUUS, kirjattu rehellisesti (Katrin oma pyyntö):** iOS:n `.ics`-käsittely PWA:sta (kotinäytölle asennettuna, "standalone"-tilassa) vaihtelee iOS-versioittain ja on tunnetusti epäluotettava — `data:`-URI valittiin `blob:`-URI:n sijaan koska `blob:` on tunnetusti ongelmallisempi juuri iOS Safari/PWA-yhdistelmässä usean version yli, mutta EI ole varmuutta että `data:`-URI toimii luotettavasti KAIKISSA iOS-versioissa/tiloissa (saattaa avata raakana tekstinä, ladata tiedostona, tai toimia suoraan — riippuu versiosta). **Playwright ei todista tätä ollenkaan** (ei voi simuloida iOS:n natiivia .ics-käsittelyä) — vain layout tarkistettu kuvakaappauksella. Katri testaa oikealla laitteella; jos avaus osoittautuu epäluotettavaksi, VAIN `avaaKalenteriSilta()`-funktiota tarvitsee muuttaa (blob:/webcal:-kokeilu) koskematta mihinkään muuhun.

**Korjaus:** `sql/067` (`ankkurit.event_date`), `api/aly-nightly.js` (tallentaa `event_date` jokaiselle osumalle), `script.js` (`icsEscape()`, `muotoileIcsAika()`, `rakennaIcsTapahtuma()`, `avaaKalenteriSilta()`, uusi nappi `loadAnchorCandidates()`:ssä). Todennettu Playwright-kuvakaappauksella (napin layout ehdokaskortilla, molemmat teemat) + `.ics`-sisällön rakenne varmistettu Node-testiskriptillä (mukaan lukien puolenyön ylitys). `sw.js` v73. **EI VIELÄ TESTATTU oikealla laitteella** — juuri tämä ominaisuus on se jonka toimivuutta ei voi todeta muuten kuin oikealla iPhonella.
