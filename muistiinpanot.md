...
- **Hetkiasia** — yksittäinen ajankohta ("hammaslääkäri ti klo 15", "palautus huomenna aamulla"). Nousee VAIN KERRAN, sen päivän aamuna. Jos käyttäjä ei reagoi, ehdotus raukeaa eikä sitä koskaan kysytä uudelleen — ei myöskään jälkikäteen ("kävitkö?"). **Hiljaisuus oli vastaus.**
...
6. Osumat → uusi `ankkurit`-rivi (`source='aly', is_candidate=true, source_ref=<murun id>`) + `aly_log`-rivi (`category`/`deadline` mukana). Ei-osumat → merkitään pysyvästi käsitellyiksi heti (ei koskaan päivämääräviitettä, ei muutu).
...
- **Asetukset, "✨ Mitä äly on tehnyt":** lista kaikista `aly_log`-riveistä uusin ensin, kukin rivi teksti+suhteellinen aika (`suhteellinenAika()`, uudelleenkäytetty) + [Kumoa]-nappi (näkyy vain jos `undone_at` on tyhjä). Kumoa poistaa ankkuririvin (jos yhä olemassa, missä tahansa tilassa — myös jo hyväksytyn/tehdyn) ja merkitsee `undone_at`:n. Kumottu rivi jää lokiin yliviivattuna, ei katoa. **Tunnettu, tietoinen rajaus:** manuaalinen "poista" (× etusivun ehdokaskortilla) EI vielä merkitse murua pysyvästi käsitellyksi (toisin kuin automaattinen raukeaminen) — jos sama muru osuu uudelleen tulevana yönä, se voi nousta uudelleen manuaalisen poiston jälkeen. Ei ollut osa raportoitua bugia (joka koski hiljaista raukeamista), joten jätetty tietoisesti tämän korjauksen ulkopuolelle — vaatisi RLS-policyn client-puolen `aly_evaluated`-kirjoitukselle, pieni mutta erillinen jatkotyö jos halutaan yhtenäistää.
