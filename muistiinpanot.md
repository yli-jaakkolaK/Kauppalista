...
- [ ] **OSA W — Tiistain koonti 2:n loppuosa, täysi uusintatesti puuttuu.** Aikavalitsimen kellonaika-tila, muistutusten kumottava siivous (Bugi 20), Laiturin "Sijoita…"-kohdevalinta (Bugi 21) ja ✨-promptidiagnoosi rakennettu 2026-07-16, EI vielä varmistettu kertaakaan oikealla laitteella.
...
## Aikavalitsin: rulla + suora syöttö + KELLONAIKA-tila (toteutettu 2026-07-16, tiistain löydösten kohta 7)
...
**Toteutus:** kaksi välilehteä (`.muistutus-tila-btn`, sama `.kalenteri-tila-btn`-visuaali uudelleenkäytettynä) saman lomakkeen sisällä — **PIKA** (ennallaan: 15/60/1440 min -pikanapit kalenteritapahtumille + suhteellinen rulla) ja uusi **KELLONAIKA**: Tänään/Huomenna-pikanapit + natiivi `<input type="date">`/`<input type="time">`. Natiivi date/time-input toteuttaa MOLEMMAT spesifioidut syöttötavat SAMALLA elementillä ilman kahta erillistä toteutusta — iOS näyttää sen omana wheel-rullanaan automaattisesti, työpöytäselaimet/Android sallivat suoran kirjoituksen. Paneeli avautuu aina PIKA-välilehdelle (ennustettava oletus, ei muuta olemassa olevaa käytöstä), KELLONAIKA-tilan päivä esitäytetään tänään:ksi ettei se jää tyhjäksi.

**Korjaus:** `index.html` (uusi välilehtirivi + kellonaika-lomake), `script.js` (välilehtivaihto, Tänään/Huomenna-esitäyttö, `#muistutus-kellonaika-lisaa-btn`-käsittelijä), `style.css` (`.muistutus-tila-valinta`, date/time-inputtien tyylit). Todennettu Playwright-kuvakaappauksella molemmissa teemoissa. `sw.js` v64.
