...
│   └── 020_tyhjenna_kalenteri_odottavat.sql — kertaluonteinen siivous: BUGI A/B -korjausta edeltäneet 15 virheellistä jonoriviä pois
...
2. **Kalenterisyötteet** (tämä osio, yllä) — max 4 päivää aikabudjetti. 017/018/019 ajettu, ensimmäinen oikea synkka onnistui (15 riviä jonoon) mutta paljasti BUGI A (hakuikkuna) ja BUGI B (RRULE-poikkeukset) — molemmat korjattu, ks. "Ensimmäisen testin löydökset". `sql/020_tyhjenna_kalenteri_odottavat.sql` AJETTAVA + uusintasynkka on seuraava askel, ks. "Testattavaa seuraavaksi".
...
- [ ] **UUSI, tekemättä: uusintasynkka korjausten jälkeen.** Aja `sql/020_tyhjenna_kalenteri_odottavat.sql` (tyhjentää vanhat 15 virheellistä riviä). Avaa Kalenteri-näkymä (käynnistää synkan) tai `GET /api/caldav-sync`. Tarkista: 1) "laivatesti joulukuussa" (Perhekalenterissa) nousee jonoon — todistaa BUGI A:n korjauksen. 2) 'Ilmoittaudu kouluun' nousee jonoon TÄSMÄLLEEN 2× (molemmat 6.8., ei kolmatta 8.8.-haamua) — todistaa BUGI B:n korjauksen. 3) Yleisesti: jonon rivimäärä on järkevä, ei selvästi liikaa/liian vähän verrattuna siihen mitä oikeasti on kolmessa kalenterissa (Perhekalenteri/Juha/Katri) seuraavan 12 kk aikana.
- [ ] **UUSI, tekemättä, VASTA YLLÄ OLEVAN JÄLKEEN: Juhan CalDAV-tili.** Vahvista ensin `?listaa=juha` toimivaksi (voi paljastaa samanlaisen 401-salasanavirheen kuin Katrin tilillä ensin oli). Sitten OMA myöhempi migraatio (`sql/021_...`, seuraava vapaa numero) samalla `on conflict do nothing` -kaavalla kuin 019:ssä, `account_key='juha'`. Avaa Kalenteri-näkymä, tarkista että Juhan tapahtuma nousee hyväksyntäjonoon täsmälleen kuten Katrinkin syötteet. **Tärkein uusi tarkistus:** jos jaettu perhekalenteri näkyy MOLEMMILLA tileillä (yksi rivi `account_key='katri'`, toinen `account_key='juha'`, molemmat osoittavat samaan jaettuun kalenteriin), sen tapahtumien pitää nousta jonoon/agendaan TÄSMÄLLEEN KERRAN, ei kahdesti. Jos jomman kumman tilin synkka epäonnistuu, tarkista Vercelin lokeista virheilmoitus per syöte — toisen tilin virhe ei saa estää toisen tilin syötteitä synkkautumasta.
