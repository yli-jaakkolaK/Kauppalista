...
**EI TESTATTU** (032 kirjoitettu, ei ajettu) — ks. PALUU.md OSA E, testi: Juha lisää menon "Oma"-kalenteriinsa → näkyy hänen Hytissään/kalenterissaan → EI näy Katrin missään näkymässä.
...
- [ ] **UUSI, tekemättä: Juhan tilin nimikorjaus + Oma-kalenterin Hytti-scope (aja `sql/031` JA `sql/032`, ks. PALUU.md OSA E) — EI TESTATTU.** Ennen 032:ta: tarkista Table Editorista että `hytti_omistajat`-taulussa on rivi `henkilo='juha'` Juhan oikealla tunnisteella. Tavoite: `GET /api/caldav-sync` näyttää 8 syötettä, 0 virhettä. Tarkista että jaettu "Yhteinen kalenteri" (Juhan tilin nimi samalle kalenterille kuin Katrin "Perhekalenteri") näkyy agendassa TÄSMÄLLEEN KERRAN, ei kahdesti — todistaa UID-duplikaattisuojan toimivan myös kahden tilin välillä. **Juhan oman "Oma"-kalenterin tapahtumat EIVÄT enää näy perheen agendassa** (scope='hytti' 032:n jälkeen) — sen sijaan Juha lisää testimenon "Oma"-kalenteriinsa, tarkista että se näkyy HÄNEN Hytissään/kalenterissaan mutta EI NÄY Katrin missään näkymässä (scope-symmetria, sama malli kuin Katrin opiskelusyötteillä). Vanhan poistetun "Katri Rantanen" -tupla-kalenterin data (jos sitä ehti tulla) pitäisi hävitä itsestään peilisäännöllä.
