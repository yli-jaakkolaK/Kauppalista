...
- **Kuormavahti** — automaattinen neutraali merkki päivälle jolla on paljon kellonaikaan sidottuja tapahtumia (raja säädettävissä Asetuksista, `asetukset`-taulun `paivan_menoraja`), ks. oma osio.
...
## Kuormavahti (2026-07-08 illalla)
...
**Yleinen `asetukset`-avainarvotaulu (`sql/023_asetukset.sql`):** `key text primary key, value text, updated_at`. Tarkoituksella yleiskäyttöinen — EI vain Kuormavahdille, tuleva sääajat/ankkurirajat-tyyppinen asetus käyttää samaa taulua. Ensimmäinen (ja toistaiseksi ainoa) rivi: `paivan_menoraja = '5'`. `value` on aina text, koodi tulkitsee (`parseInt`) — ei erillisiä sarakkeita eri tyypeille tässä mittakaavassa. RLS: `for all using (auth.uid() is not null)`, sama yksinkertainen malli kuin `kalenteri_syotteet`.

**Laskenta (`script.js`):**
- `paivitaAsetukset()` hakee koko `asetukset`-taulun kerran per Kalenteri-näkymän lataus välimuistiin (`asetuksetKartta`), `haeAsetusNumero(key, oletus)` lukee ja parsii sen.
...
**Näkyvyys:**
- Päivänäkymässä: `#kalenteri-otsikko` (sivun ylälaidan päiväteksti) saa perään `.kalenteri-kuorma-merkki`-tagin ("⚑ N") jos päivän menomäärä ≥ raja. `paivitaOtsikkoKuormamerkilla()`.
- Viikkonäkymässä: sama tagi jokaisen päivän omaan `.kalenteri-paiva-otsikko`-otsikkoon, laskettuna sen päivän omista riveistä (`piirraKalenteriPaivaRyhma()`:n uusi 4. parametri `kuormaraja`).
- Kuukausiruudukko EI saa merkkiä (ei pyydetty, solu on liian pieni).
- Tyyli: sama neutraali `.kalenteri-uusi-merkki`-henkinen dashed-border-accent-badge — EI punaista, EI syyllistävää tekstiä, vain `"⚑ N"` + `title`-attribuutti selityksellä hiiren yli (tai pitkän painalluksen tooltip-korvike mobiilissa, ei kriittinen).

**Asetukset-näkymä:** uusi "🚦 Kuormavahti" -osio (`#kuormaraja-input`, numerokenttä, `min=1`). Tallentuu `asetukset`-tauluun `change`-tapahtumassa (ei jokaisesta näppäinpainalluksesta) — **YHTEINEN koko perheelle** (ei laitekohtainen `localStorage`, toisin kuin Hytti-kalenteri-kytkin), koska kyse on kalenterin yhteisestä kuormasta jonka molempien pitää nähdä samana.

**Kuittausjono-kytkös (kohta 4, Katrin speksistä):** `avaaKuittausOverlay()` laskee JOKA RIVILLE (kun overlay avataan, ei synkkauksen aikana palvelimella) kuinka monta MUUTA kellonaikamenoa samalla `event_date`:llä on (`laskeMuutaMenoaPaivalle()`, yksi kysely per rivi, rinnakkain `Promise.all`:lla — pieni perhemittakaavan taulu, ei suorituskykyongelmaa). Jos määrä ≥ raja, kortille lisätään pieni huomiorivi `"huom: päivällä jo N muuta menoa"` (`.kalenteri-kortti-kuorma`, accent-väri). **Tietoinen suunnittelupäätös:** laskettu AINA TUOREENA overlayn avaushetkellä (ei tallennettu synkkaushetken tilaa erikseen) — yksinkertaisempaa kuin seurata "oliko tämä juuri uusi TÄSSÄ synkkauksessa", ja antaa aina ajantasaisen kontekstin riippumatta milloin rivi oikeasti kuitataan.
- **Ei estämistä, ei lisäyshetken varoitusta** (ks. Katrin oma perustelu): lisäys tapahtuu iCalissa jota Satama ei voi keskeyttää — vahti toimii pelkän näkyvyyden kautta.

**Ei testattu ollenkaan tätä kirjoittaessa** — aja `sql/023_asetukset.sql`, testaa: 1) lisää/synkkaa 5+ kellonaikamenoa samalle päivälle, tarkista merkki agendassa ja viikossa, 2) vaihda raja Asetuksista, tarkista että merkki ilmestyy/katoaa sen mukaan koodimuutoksetta, 3) synkkaa uusi tapahtuma kuormitetulle päivälle, tarkista kuittausjonon kortin lisähuomio.
...
### Ajolista #2 — arkkitehtuurimuutos "yksi totuus, kaksi ikkunaa" (kirjattu 2026-07-08 illalla, VIELÄ AJAMATTA — aja seuraavaksi)
...
- Kalenterin otsikko + päivämäärä isommaksi mobiilissa
- Päivämäärälle/kellonajalle oma erottuva väri agendassa (nyt `var(--muted)`, sama kuin muu toissijainen teksti)
