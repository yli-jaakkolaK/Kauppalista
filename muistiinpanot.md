...
Sama `laituri.status`-tilakenttä kuin OSA A:n "siirretty Kauppalistalle" (`sijoitettu`) — `arkistoitu` on sen kolmas arvo, EI erillinen mekanismi/sarake. ⋯-valikkoon (`createOverflowButton`) lisätty "🗄 Arkistoi" jokaiselle Laiturin riville (ei omaksi napiksi — rivillä on jo enimmillään viisi nappia, sama tilanahtaus-oppi kuin aiemmin ankkuri-rivillä). `arkistoiLaituriRivi()` päivittää tilan, `lataaLaituri()`:n kysely suodattaa arkistoidut pois (`.neq('status', 'arkistoitu')`) — muru EI KATOA, vain piiloutuu aktiivisesta näkymästä JA (koska `ai_evaluated`-merkintä pysyy) älyn nostokandidaateista. Kevyt arkistonäkymä (Katrin oma rajaus: "pelkkä lista riittää") — uusi "Arkisto"-linkki Laiturin alaosassa (näkyy vain kun arkistoituja on, sama malli kuin Hytin `paivitaHyttiArkistoLinkki()`) avaa `#laituri-arkisto-overlay`-dialogin listana, jokaisella rivillä ↺-palautusnappi (`palautaLaituriArkistosta()` — status takaisin `'uusi'`:ksi, `placed_where` nollataan).
...
**Korjaus:** `sql/070` (uusi migraatio: `laituri_status_check` laajennettu `arkistoitu`-arvolla, `laituri.ai_kauppa_ehdotus` jsonb-sarake), `api/_lib/aly-classify.js` (uusi, jaettu moduuli), `api/aly-nightly.js` (kauppa-luokka + laukaisusana-korjaus), `api/laituri-add.js` (heti-luokittelu + laukaisusana-regex-korjaus), `script.js` (kauppaehdotuskortti + hyväksyntä/hylkäys, arkistointi/palautus/arkistonäkymä, laukaisusana-regex-korjaus), `index.html` (Arkisto-linkki + -overlay), `style.css` (`.laituri-arkisto-rivi`), `sw.js` v75. Testirivit `sql/071` (OSA X). **EI TESTATTU OIKEALLA LAITTEELLA — ja äly-luokittelun 8 testitapausta EI TESTATTU OIKEALLA API-KUTSULLA** (ks. yllä, ei `ANTHROPIC_API_KEY`:tä tässä ympäristössä).
