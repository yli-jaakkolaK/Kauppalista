...
## Jatka tästä (päivitetty 2026-07-13 illalla)
...
**BUGI KORJATTU: listan poisto ei toiminut** (`sql/039` + `poistaLista()`-koodikorjaus) — ks. "Bugikorjaus: Listan poisto ei toiminut" -osio. AJA `sql/039` ennen kuin testaat listan poistoa. EI TESTATTU vielä oikealla laitteella.

Jos joudut päättämään mistä jatkat: aja ensin PALUU.md:n VAIHE 1 -testit läpi (täppäys Satama-listalta), sitten torstaina VAIHE 2 Juhan kanssa, ja vasta sen jälkeen aloita ristiriitapaketti.
...
**Oire:** Katri yritti poistaa käsin luotua listaa ("jäätelökakku", Muistilaput) — vahvistusdialogi kysyi, hän hyväksyi, mitään ei tapahtunut. Toistettavissa, ei kertaluontoinen: sama tulos aiemminkin. Katrin pyynnöstä tutkittu `poistaLista()`-ketju päästä päähän eikä vain oletettu regressioksi.

**Juurisyy (kaksi kerrosta):**
1. `poistaLista()` (script.js) poistaa oikeassa FK-turvallisessa järjestyksessä: `tuotteet` → `events` → `lists` (sama järjestys jo kirjattu 2026-07-06, ks. "Tunnettuja asioita" yllä).
