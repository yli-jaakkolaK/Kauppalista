...
## Äly-putki (2026-07-11, ensimmäinen kerros — EI vielä yhtään oikeaa älyominaisuutta)

Todistettu putki puhelimesta Claude API:iin ja takaisin, jonka päälle jokainen tuleva älyominaisuus (Siri-tulkinta, Laituri-luokittelu, jääkaappikuva) rakentuu — sama kaava kuin push-infrassa (ks. "Push-ilmoitukset"-osio): putki todistetaan ensin erikseen, ominaisuudet perässä. **Täysi tekninen kuvaus ja "miten lisään uuden ominaisuuden" -ohje on COPILOT.md:ssä** — tämä on uusi tiedosto nimenomaan tätä tarkoitusta varten (tekninen jatko-opas, eri asia kuin tämä narratiivinen muistiinpanot.md), lue sen oma yläkommentti erosta.
...
**EI TÄHÄN KERROKSEEN (kirjattu, ei rakennettu):** ei yhtään oikeaa älyominaisuutta, ei prompteja datalle, ei kuvatukea. Nämä suunnitellaan erikseen kun putki on todistettu toimivaksi.

**EI TESTATTU LAINKAAN** — vaatii `ANTHROPIC_API_KEY`-ympäristömuuttujan Verceliin, ks. PALUU.md OSA F.
...
2. `sql/027_kalenteri_syotteet_scope.sql` — `kalenteri_syotteet.scope` (`'perhe'`/`'hytti'`, oletus `'perhe'`) + **uusi `hytti_omistajat`-taulu** (henkilo→user_id-kartta, Katrin rivi valmiina, Juhan rivi lisättävä Table Editorista) + **`kalenteri_tapahtumat`-taulun RLS-politiikan korjaus** (ks. alla, kriittinen yksityisyyskorjaus jota alkuperäinen speksi ei vielä eritellyt).
...
- [ ] **UUSI, tekemättä: Äly-putken runko (aseta `ANTHROPIC_API_KEY` Verceliin, ks. PALUU.md OSA F) — EI TESTATTU LAINKAAN, riippumaton kaikesta muusta.** Avaa Asetukset → Sovellus → paina "Testaa äly" — pitäisi näkyä järkevä suomenkielinen vastaus napin alla muutaman sekunnin sisällä. Jos virhe, lue viesti (se on tarkoituksella selkokielinen) — todennäköisin syy puuttuva/väärin kirjoitettu `ANTHROPIC_API_KEY` tai mallitunniste jota Anthropic ei tunnista (tarkista `ALY_MALLI`). Ei vielä yhtään oikeaa älyominaisuutta testattavana — pelkkä putki.
...
- [ ] **UUSI, tekemättä: Juhan CalDAV-tilin syöterivit (aja `sql/030_kalenteri_syotteet_data_juha.sql`, ks. PALUU.md OSA E) — EI TESTATTU LAINKAAN.** Vahvista ENSIN `?listaa=juha` toimivaksi ja täsmää tulos migraation `tunniste`-arvoihin (voi paljastaa samanlaisen 401-salasanavirheen kuin Katrin tilillä ensin oli). Avaa Kalenteri-näkymä, tarkista että Juhan tapahtuma näkyy agendassa suoraan täsmälleen kuten Katrinkin syötteet, "uusi"-tagilla jos tekijää ei tunnisteta Katriksi. **Tärkein tarkistus:** jos jaettu perhekalenteri näkyy MOLEMMILLA tileillä (yksi rivi `account_key='katri'`, toinen `account_key='juha'`, molemmat osoittavat samaan jaettuun kalenteriin), sen tapahtumien pitää näkyä agendassa TÄSMÄLLEEN KERRAN, ei kahdesti — todistaa UID-duplikaattisuojan toimivan myös kahden tilin välillä (verifioitu lukemalla koodi ennen ajoa, ei vielä oikealla datalla). Jos jomman kumman tilin synkka epäonnistuu, tarkista Vercelin lokeista virheilmoitus per syöte — toisen tilin virhe ei saa estää toisen tilin syötteitä synkkautumasta.
