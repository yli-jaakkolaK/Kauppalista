...
**Tietomalli (`sql/016_hytti.sql`, EI VIELÄ AJETTU Supabasessa):**
...
**Ei testattu ollenkaan tätä kirjoittaessa** — `sql/016_hytti.sql` ei ole ajettu Supabasessa. Testaa kahdella tilillä (Katri + Juha): kumpikin luo oman kortin, kumpikaan EI näe toisen kortteja/tehtäviä missään näkymässä (ei päänäkymässä, ei Ankkureissa, ei Tehtävät-koosteessa).
...
6. **Oma Hytti -runko** — ✓ TOTEUTETTU 2026-07-08 v1 (casekortit + automaattinen tehtäväkooste, täysin yksityinen), ks. "Oma Hytti"-osio. `sql/016_hytti.sql` AJAMATTA, EI testattu ollenkaan vielä (myös kahden tilin RLS-testi Hytille erikseen kohdassa 1 mainitun lisäksi)
...
- [x] **Oma Hytti** — toteutettu 2026-07-08 v1 (casekortit + tehtäväkooste, täysin yksityinen), ks. "Oma Hytti"-osio. `sql/016_hytti.sql` AJAMATTA, EI vielä testattu. Ohjebanneri-järjestelmä (oma täysi suunnitelma valmiina, tarkoitettu nimenomaan Hytin ensimmäiseksi käyttökohteeksi) EI ole vielä mukana — jäi tietoisesti pois v1:stä, rakennetaan myöhemmin päälle
