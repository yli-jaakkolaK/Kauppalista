...
**Kolme paikkaa jotka piti PITÄÄ ennallaan samalla kun näkyvyys avattiin** (Katri pyysi vain agenda/viikko/kuukausi-näkyvyyttä, ei näitä):
1. **Kuittausjono** (`paivitaKuittausTila()`) — suodattaa hytti-scopen edelleen pois erikseen, koskematon.
2. **Kuormavahti** (`laskeMenoja()`) ja **ristiriitamerkki** (`onkoPaivanRistiriita()`) — molemmat saivat uuden `r._scope !== 'hytti'` -ehdon suodattimeensa, koska muuten oma opiskelu/työmeno olisi alkanut laskea perheen kuormaan/ristiriitoihin mukaan pelkän näkyvyyskorjauksen sivuvaikutuksena.
3. **Rivikohtainen "🆕 uusi" -merkki agendassa** (`onkoUusiMinulle()`) — LÖYDETTY piilevä riski ennen kuin se pääsi tuotantoon: Lukkarikone/Itslearning-organizereita ei tunneta `kalenteri_tekijat`-kartassa, joten näiden rivien `user_id` jää NULLiksi kannassa — ilman erillistä hytti-poikkeusta jokainen oma opiskelutapahtuma olisi näyttänyt VÄÄRIN "uudelta" ja tarjonnut yksittäisen kuittausnapin, vaikka koko pointti on ettei hytti kuulu kuittausjärjestelmään lainkaan.

**Korjaus:** `script.js` — `lataaKalenteri()`:n blanketti-hytti-suodatus poistettu, `_scope` lisätty mapattuun tapahtumaolioon (myös päivänäkymän `rivit`-mappaukseen), `laskeMenoja()`/`onkoPaivanRistiriita()`/`onkoUusiMinulle()` saivat eksplisiittisen `scope!=='hytti'`-poissulun. Ei uutta migraatiota (RLS jo riitti). `sw.js` v61. Uusintatestirivit `sql/054`, testiaskeleet PALUU.md OSA U:ssa. **EI TESTATTU vielä oikealla laitteella/datalla** — testikriteeri: Katri selaa syyskuuhun → luennot näkyvät kalenterissa muttei kuittausjonossa/Kuormavahdissa; Juha selaa samaan ajankohtaan → ei näe niitä.
