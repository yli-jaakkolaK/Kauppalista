...
- **Laituri-laatta** = TOISEN käyttäjän lisäämät `status='uusi'`-rivit (`.eq('status','uusi').neq('user_id', currentUserId)`) — oma lisäys EI kerrytä omaa palluraa, koska ei tarvitse muistutusta omasta juuri kirjoittamastaan rivistä.
...
**Reagointi ≠ nähty:** pallura katoaa kun asiaan REAGOIDAAN (kuittaus / sijoitus), ei kun laatta/näkymä vain avataan. Tämä on tietoinen ero vanhaan Laiturin "nähty"-aikaleimamalliin nähden (ks. "Laituri"-osio yllä, historiaosuus) — todettiin liian löysäksi jo aiemmassa testisessiossa.
