...
- [ ] **UUSI, tekemättä: uusintasynkka "yksi totuus, kaksi ikkunaa" -mallilla.** Aja `sql/021_kalenteri_kuittausjono.sql` ja `sql/022_kalenteri_puhdas_alku.sql` (ks. "Ajolista #2" Kalenterisyötteet-osiossa, korvaa vanhentuneen `sql/020`-pohjaisen ohjeen). Avaa Kalenteri-näkymä (käynnistää synkan) tai `GET /api/caldav-sync`. Tarkista: 1) "laivatesti joulukuussa" (Perhekalenterissa) näkyy AGENDASSA SUORAAN (ei enää jonossa) — todistaa BUGI A:n korjauksen. 2) 'Ilmoittaudu kouluun' näkyy TÄSMÄLLEEN 2× (molemmat 6.8., ei kolmatta 8.8.-haamua) — todistaa BUGI B:n korjauksen. 3) Toisen tililtä/kalenterista tulleet tapahtumat saavat "uusi"-tagin agendassa + "🆕 N uutta" -linkin, omat (tai tekijättömät jos organizer ei toimi) käyttäytyvät suunnitelman mukaan. 4) "✓ Kuittaa" ja "Kuittaa kaikki" toimivat, tagi häviää. 5) Muokkaa/poista yksi testitapahtuma iPhonen Kalenterista, synkkaa uudelleen — muutos/poisto näkyy Satamassa (peilisääntö). 6) `GET /api/caldav-sync?esikatsele=1` — tarkista `organizer`-kentän arvot, kerro Claudelle/Copilotille tulos jotta `kalenteri_tekijat` osataan täyttää tarvittaessa.
- [ ] **UUSI, tekemättä: Kuormavahti (aja `sql/023_asetukset.sql` ensin, riippumaton muista).** Avaa Asetukset, tarkista että "🚦 Kuormavahti" -kentässä näkyy oletusarvo 5. Lisää/synkkaa (tai käsin `kalenteri_tapahtumat`-tauluun) 5 kellonaikaan sidottua tapahtumaa samalle päivälle — avaa Kalenteri päivänäkymässä, tarkista että päiväotsikon perässä näkyy "⚑ 5" -merkki. Vaihda Kalenteria viikkotilaan, tarkista että sama päivä saa merkin siellä (muut päivät ei). Lisää koko päivän tapahtuma (esim. syntymäpäivä) samalle päivälle — tarkista että se EI nosta laskuria (pysyy "⚑ 5", ei "⚑ 6"). Vaihda raja Asetuksista esim. 3:een, tarkista että merkki nyt näkyy myös harvemmilla päivillä ilman koodimuutosta/uudelleenkäynnistystä. Synkkaa uusi tapahtuma kuormitetulle päivälle (5+) ja avaa kuittausjono — kortilla pitäisi näkyä "huom: päivällä jo N muuta menoa".
