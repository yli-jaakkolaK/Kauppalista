...
├── .github/workflows/
│   └── muistutukset-cron.yml     — GitHub Actions -ajastin (5 min välein), kutsuu muistutukset-laheta.js:ää JA caldav-sync.js:ää (ks. "Muistutukset"-osio)
├── sql/
...
## Laituri (2026-07-07, badge-semantiikka + luonnos-varmuusverkko korjattu 2026-07-10)
...
**Kaksi ERI lukua, kaksi eri kysymystä (korjattu 2026-07-10, ks. "Kahden puhelimen testisessio"):** Katri huomasi ettei etusivun pallukka nollaantunut Laiturin avaamisesta — se laski aiemmin KAIKKI `status='uusi'` (käsittelemättömät) rivit, riippumatta siitä oliko ne jo nähnyt. Nyt kaksi erillistä lukua vastaavat kahteen eri kysymykseen:
- **Etusivun pallukka = "mitä en ole vielä NÄHNYT"** — laitekohtainen (localStorage-avain `satama_laituri_viimeksi_avattu`, sama malli kuin Hytti-kalenteri-kytkimellä, koska Katrilla ja Juhalla on omat puhelimensa). Laskee rivit joiden `created_at` on TÄMÄN laitteen viimeisimmän Laiturin-avauksen jälkeen (`.gt('created_at', viimeksiAvattu)`). Nollaantuu kun Laituri avataan (`merkitseLaituriNahdyksi()`, kutsutaan `avaaOsio()`:n laituri-haarasta). Jos avaamista ei ole vielä koskaan kirjattu tälle laitteelle, oletusarvo on epoch (1970) eli KAIKKI nykyiset rivit näkyvät kertaalleen "uusina" ensimmäisellä avauskerroista tämän muutoksen jälkeen — kertaluonteinen, odotettu sivuvaikutus.
- **Laituri-näkymän sisäinen teksti (`#laituri-sijoittamatta`) = "mitä ei ole vielä KÄSITELTY"** — laskee edelleen `status='uusi'`-rivit riippumatta siapa on nähnyt ne, näkyy hakukentän alla tekstinä ("N sijoittamatta" / "kaikki sijoitettu"), päivittyy joka kerta kun `lataaLaituri()` ajetaan (myös hakusuodatuksesta riippumatta — laskee AINA kokonaismäärän, ei suodatettua listaa). Rivien oma tummennus (opacity 0.5) + "→ minne"-teksti näyttävät saman tiedon myös rivikohtaisesti.
...
3. **💡 Vinkit** — **staattinen** lista (`.vinkki-lista`/`.vinkki-rivi`, suoraan `index.html`:ssä, EI tietokantataulua) yhdeksästä käyttövihjeestä. Tietoinen tilapäisratkaisu: kun Ohjebanneri-järjestelmä (ks. oma suunnitelma-osio) joskus rakennetaan, sama sisältö siirtyy `ohjeet`-tauluun ja tämä lista muuttuu siitä luettavaksi — EI ole tehty vielä, koska `ohjeet`-taulua ei ole olemassa (Ohjebanneri on vain suunniteltu, ei toteutettu).
...
5. **🚪 Oma Hytti** (ei osa Katrin alkuperäistä 4-osion speksiä, mutta säilytetty listan lopussa) — Kalenteri-integraation pois/päälle-kytkin, ks. "Oma Hytti"-osio.
...
## Hytti v1 + opiskelulaajennus + ICS-syötekoneisto — LOPULLINEN SPEKSI (kirjattu 2026-07-11, EI ALOITETTU)

**TÄRKEÄ PORTTIEHTO — älä aloita tätä ennen kuin KAIKKI kolme pätee:** (a) GitHub-tokenin `workflow`-oikeusongelma ratkaistu ja Muistutukset v1 (`d3bac23`) pushattu+deployattu Verceliin, (b) `sql/025_muistutukset.sql` ajettu Supabasessa, (c) muistutustesti läpäisty oikealla laitteella (push saapui suljettuun puhelimeen). Tämä on Katrin oma eksplisiittinen järjestys 2026-07-11 — täysin erillinen saareke, EI kosketa kuittausjonoa/agendaa joiden rästitestit (peilikuvatesti, Löydös 3 jne., ks. "Kahden puhelimen testisessio") ovat yhä kesken. Kun portti aukeaa, tee OSA 1 → OSA 2 samassa paketissa (OSA 3 sovelletaan heti Hyttiin, muihin näkymiin sitä mukaa kun niitä muutenkin kosketaan).

### OSA 1: Hytti v1 -respeksaus (laajentaa 2026-07-08 rakennettua, ei aloita alusta)

Olemassa oleva `hytti_kortit`/`hytti_rivit` (ks. yllä, `sql/016_hytti.sql`) kattaa jo suurimman osan — **skeemadelta on pieni**:
- `hytti_kortit` tarvitsee UUDEN sarakkeen `kalenterisuodatin` (text, null ok) — kaikki muu (`owner_id`, `name`, `card_type`, `status`, `seuraava_askel`, `sort_order`) on jo olemassa.
- `hytti_rivit` ei tarvitse MITÄÄN uutta saraketta — `is_task`/`done`/`done_at`/`due_date`/`is_header` ovat jo kaikki paikallaan.
- **`muistutukset`-taulun check-rajoite** (`sql/025`) sallii tällä hetkellä vain `source in ('rivi','kalenteri','ankkuri')` — tarvitsee ALTER TABLE:n joka lisää `'hytti_rivi'` sallittujen joukkoon ennen kuin Hytin rivien kello-nappi voi toimia.

**Uudet/muuttuneet osat päänäkymässä (ks. olemassa oleva "Oma Hytti" -kuvaus yllä perustaksi):**
- **Työ/vapaa-kytkin siirtyy Asetuksista Hytin päänäkymän yläreunaan** ("Opiskelu näkyvissä / piilossa" -teksti). Koskee VAIN näkyvyyttä ARKINÄKYMISSÄ (etusivu, kalenteri) — Hytin SISÄLLÄ oma scope näkyy AINA riippumatta kytkimen tilasta. Todennäköisesti sama laitekohtainen `localStorage`-malli kuin nykyinen `kauppalista_hytti_kalenterissa`-kytkin (ei synkattu Supabaseen), mutta harkitse ajaessa onko tämä nyt PALJON isompi asia (piilottaa myös ICS-syötteen tapahtumia, ei vain tehtäviä) — jos kyllä, saattaa tarvita oman Supabase-tallennuksen jos halutaan sama tila molemmilla laitteilla. **Päätä toteutusvaiheessa, kirjaa valinta.**
- **UUSI "Tänään-kaista":** päivän hytti-scopen (ks. OSA 2) kalenteritapahtumat kellonaika+nimi, TYHJÄNÄ ei näy ollenkaan (ei tyhjätilatekstiä tälle yksittäiselle lohkolle).
- **Tehtävät-kooste laajenee:** rivillä nyt myös KELLO-nappi (`luoMuistutusNappi('hytti_rivi', rivi.id, ...)`, sama komponentti kuin muualla) ⚓-napin vieressä.
- **Kortit-listaus:** ennallaan.
- **Uusi kortti -selite päivitetty sanamuoto:** "päättyvä = valmistuu ja arkistoituu, jatkuva = elää pitkään" (pieni sanamuodon tarkennus edellisestä).
- **Uusi tyhjätilateksti (korvaa 2026-07-08 version):** *"Yksi kortti per kurssi, projekti tai tilaisuus. Kirjoita muistiinpanot riveiksi, merkkaa toimintaa vaativat tehtäviksi — ne kokoontuvat itsestään ylös. Pidä seuraava askel pienenä ja rumana: ei 'kirjoita essee' vaan 'avaa tiedosto ja kirjoita yksi ruma kappale'. Ideat kuuluvat Laituriin — työn alla olevat tänne."*

**Korttinäkymän UUSI järjestys (Katrin eksplisiittinen linjaus, korvaa nykyisen):**
1. Otsikko + tyyppi
2. **UUSI OSIO — Kortin kalenteri:** tulevat hytti-scopen tapahtumat joiden OTSIKKO sisältää kortin `kalenterisuodatin`-arvon (case-insensitive `ilike '%...%'`, EI mitään älyä/fuzzy-hakua). Tyhjä suodatin TAI ei osumia → osio ei näy ollenkaan. Näytä esim. 7 päivää eteenpäin (kiinteä ikkuna, ei asetus v1:ssä).
3. **Seuraava askel:** UUSI ULKOASU — kehystetty laatikko kulta-reunuksella (ei enää pelkkä rivi otsikon alla), napautus = inline-muokkaus, placeholder "seuraava pieni askel...".
4. Rivit — ennallaan (lisäys/muokkaus/poisto, `#`-väliotsikot). **UUSI:** rivin merkitseminen tehtäväksi tapahtuu jollain kitkattomalla tavalla joka EI ole aina näkyvä nappi (Katrin oma huomautus: "toteutustapa Coden harkinta, esim. pitkä painallus -valikko — kirjoittamisen pitää pysyä kitkattomana") — päätä toteutusvaiheessa, esim. pitkä painallus avaa pienen valikon ("Merkitse tehtäväksi" + valinnainen eräpäivä). Tehtäväriveillä ⚓+KELLO, muistiinpanoriveillä ei kumpaakaan. **Tehty tehtävä EI katoa listalta** — jää näkyviin yliviivattuna (kurssihistoria on arvokasta, poikkeaa esim. Muistilappujen "ostettu"-käytöksestä tarkoituksella).
5. "Arkistoi kortti" päättyvällä kortilla — ennallaan. Kaikesta Events-lokitus (`logEvent()`, sama yleinen mekanismi kuin muuallakin).

### OSA 2: ICS-syötekoneisto (laajentaa `kalenteri_syotteet`/`api/caldav-sync.js`)

**Uusi syötetyyppi `tyyppi='ics'`** (rinnalle `'icloud'`/`'ics_url'`, ks. "Kalenterisyötteet"-osio): **`tunniste` on tällä kertaa ENV-MUUTTUJAN NIMI, ei suora URL** — koska nämä .ics-linkit sisältävät henkilökohtaisen tokenin URL:ssaan (tyypillinen kalenteriliitäntöjen tapa), niitä ei saa tallentaa suoraan tauluun samaan tapaan kuin julkisen `ics_url`-syötteen linkki. `api/caldav-sync.js` lukee tunnisteen NIMEÄMÄN Vercel-ympäristömuuttujan arvon vasta ajon aikana. Vercelissä valmiina jo (Katrin mukaan): `ITSLEARNING_ICS_KATRI` ja `LUKKARIKONE_ICS_KATRI` (jälkimmäinen `http://`-alkuinen — hyväksytty, ei vaadita https:ää tälle syötetyypille).

**Haku:** täsmälleen sama putki kuin CalDAV-syötteillä — `fetch(url)` → sama `ical.js`-jäsennys + `getOccurrenceDetails()` BUGI B -korjauksen kanssa → sama `PAIVIA_TAAKSEPAIN`/`PAIVIA_ETEENPAIN`-aikaikkuna → UID-pohjainen duplikaattisuoja → peilisääntö (`merge-duplicates`-upsert + `siivoaPoistetut()` PER SYÖTE). Ei siis omaa erillistä koodipolkua, vain uusi haaraus "miten tunniste muutetaan oikeaksi URL:ksi" -kohtaan (icloud: CalDAV-kirjautuminen; ics_url: tunniste ON url; **ics: `process.env[syote.tunniste]` ON url**).

**UUDET SARAKKEET `kalenteri_syotteet`-tauluun:**
- `scope` (text, `'perhe'`/`'hytti'`, default `'perhe'` — nykyiset rivit migroituvat automaattisesti perheeksi, ei manuaalista taannehtivaa päättelyä)
- `owner` (uuid tai `henkilo`-tyylinen text-viittaus, käytössä VAIN `scope='hytti'`-riveillä — kertoo kenen Hyttiin tapahtumat kuuluvat)

**SCOPE-SÄÄNNÖT (tarkka, ei tulkinnanvaraa toteutusvaiheeseen):**
- `scope='perhe'` → nykykäytös kaikkialla, ei muutu mihinkään (kuittausjono, agenda, Kuormavahti, Ristiriitamerkki — kaikki koskevat vain tätä).
- `scope='hytti'` → näkyy VAIN: (1) omistajan Hytin tänään-kaistalla, (2) omistajan hytti-korttien "Kortin kalenteri" -osiossa (`kalenterisuodatin`-osuma). EI KOSKAAN: kuittausjonoon (omaa opiskelua ei kuitata — ei ole "uusi tälle toiselle käyttäjälle" -käsitettä koska toinen ei näe sitä ollenkaan), toisen käyttäjän MIHINKÄÄN näkymään, Kuormavahtiin (v1-yksinkertaistus: **Kuormavahti laskee VAIN `scope='perhe'`-tapahtumat**, kirjattu tietoisena rajauksena — opiskelukalenteri voi olla täynnä ilman että se "kuormittaa" perheen yhteistä näkymää). Työ/vapaa-kytkin piilottaa hytti-scopen tapahtumat ARKINÄKYMISTÄ (etusivu, perhekalenteri) muttei koskaan itse Hytistä.

**Syöterivit (osa migraatiota kun toteutusvaihe alkaa):** kaksi uutta riviä — Itslearning (`tyyppi='ics'`, `tunniste='ITSLEARNING_ICS_KATRI'`, `scope='hytti'`, `owner=`Katri) ja Lukkarikone (sama malli, `tunniste='LUKKARIKONE_ICS_KATRI'`). `enabled=true`. Sama `on conflict do nothing` -idempotenttiuskuvio kuin `sql/019`.

**KIRJATTU MYÖHEMMÄKSI (ei tässä paketissa):** Juhan työkalenteri samaan `tyyppi='ics'`-koneistoon, mutta `mode='vain_varattu'` PERHEEN puolelle (näkyy perhekalenterissa vain "🔒 Varattu HH–HH") ja täydet tiedot Juhan omassa Hytissä — vaatisi tarkemman mietinnän siitä miten yksi syöte voi olla SEKÄ vain_varattu (perheelle) ETTÄ täysi (omistajan Hytille) samaan aikaan; nykymalli (`mode` per syöterivi) ei suoraan taivu tähän, saattaa tarvita KAKSI riviä samalle URL:lle (yksi scope='perhe' mode='vain_varattu', toinen scope='hytti' mode='taysi') — ratkaistaan kun asiaan tullaan.

### OSA 3: Värisääntö — "väri koristaa, musta kertoo" (design-periaate, koko sovellus)

Uusi, KOKO SOVELLUSTA koskeva periaate, täydentää 2026-07-10 WCAG-kontrastikorjausta (ks. "Ulkokäytettävyys ja kontrasti" -osio): **informaatiota kantava teksti (kellonajat, päivämäärät, sisällöt, mikä tahansa jota luetaan tiedoksi) käyttää AINA täyttä `var(--text)`-tekstiväriä molemmissa teemoissa.** `--accent`/`--muted` EIVÄT KOSKAAN ole ainoa tiedon kantaja — ne elävät reunuksissa, kehyksissä, katkoviivoissa ja osiotunnisteissa (asioissa jotka tunnistaa MUODOSTA lukematta, esim. `.kalenteri-tila-btn.active`:n kulta-reunus, `.section-title`:n väri otsikkotasona). **Käytännön vaikutus jo olemassa olevaan koodiin:** esim. `.kalenteri-aika` sai 2026-07-10 oman `--accent-text`-värinsä "ajalle oma väri" -pyynnöstä — tämä UUSI sääntö kyseenalaistaa sen: kellonaika ON informaatiota (milloin tapahtuma on), joten sen pitäisi tämän periaatteen mukaan olla `--text`, ei `--accent-text`. **Ristiriita kirjattu, EI korjattu automaattisesti** — päätä toteutusvaiheessa kumpi periaate voittaa (todennäköisesti: `--accent-text` oli nimenomaan KONTRASTIKORJAUS informaatiolle joka ennen oli `--muted` [liian heikko], joten se on jo linjassa "informaatio=täysi luettavuus" -hengen kanssa vaikka ei olekaan kirjaimellisesti `--text`; tämän uuden säännön kirjaimellinen tulkinta veisi kohti pelkkää `--text`:iä — harkitse kumpi luettavampi ulkona, älä vaihda ilman testiä). **Sovella Hyttiin HETI kun se rakennetaan (uusi näkymä, ei painolastia), muihin näkymiin VASTA sitä mukaa kun niitä muutenkin kosketaan** — ei erillistä koko-sovelluksen läpikäyntiä pelkästään tämän vuoksi.

### EI V1:EEN (kirjattu, ei rakenneta tässä paketissa)

Ohjebanneri-koneisto (tyhjätila riittää edelleen), haku, rivien siirto korttien välillä, Laituri→Hytti-sijoitus, Opiskelu-AI-toiminnot, hytin nimenvaihto, "seuraava askel" tehtäväksi -mekaniikka (askel pysyy pelkkänä tekstinä v1:ssä, ei oma checkbox/due_date-pari).

### Testaus (kun toteutusvaihe alkaa)

**HUOM kausiluonteisuus:** opiskelusyötteet (Itslearning/Lukkarikone) ovat KESÄLLÄ tyhjiä tai lähes tyhjiä — nyt tehtävä on vain PUTKITESTI (syöte haetaan virheettä, 0–N tapahtumaa tulee sisään `scope='hytti'`-leimalla, eivät näy Juhalle eivätkä missään jonossa), varsinainen SISÄLTÖTESTI vasta elokuussa kun lukujärjestys/kurssit ovat oikeasti käynnissä — **kirjattava erikseen testauslistalle uudelleen elokuussa**, ei riitä että putkitesti läpäistiin heinäkuussa.

**Hytti-putkitesti:** Katri luo "Projektikurssi"-kortin (aseta myös `kalenterisuodatin`) → lisää muistiinpanoja + 2 tehtävää eräpäivillä → Tehtävät-kooste näyttää ne päivinä laskettuna → täppäys koosteesta täppää myös kortilla → ⚓-nosto etusivun Ankkureihin toimii → kello-muistutus jollain hytti-rivillä tuottaa oikean pushin (todistaa `muistutukset.source='hytti_rivi'` toimii check-rajoitteen laajennuksen jälkeen) → **Juha ei näe MITÄÄN tästä omalla tilillään** (ei korttia, ei tehtävää, ei kalenteritapahtumaa, ei kuittausjonossa).

**Kerro lopuksi (kun toteutusvaihe päättyy):** migraationumerot ajojärjestyksessä (todennäköisesti `sql/026_hytti_kalenterisuodatin.sql` + `sql/027_muistutukset_hytti_rivi.sql` + `sql/028_kalenteri_syotteet_scope.sql` + `sql/029_hytti_ics_syotteet_data.sql`, tarkka numerointi ja mahdollinen yhdistely päätetään toteutusvaiheessa), mitä Katri ajaa missä järjestyksessä, ja täsmällinen testiohje.
...
- [ ] **UUSI, tekemättä, VASTA YLLÄ OLEVAN JÄLKEEN: Juhan CalDAV-tili.** Vahvista ensin `?listaa=juha` toimivaksi (voi paljastaa samanlaisen 401-salasanavirheen kuin Katrin tilillä ensin oli). Sitten OMA myöhempi migraatio (`sql/024_...`, seuraava vapaa numero 023:n jälkeen) samalla `on conflict do nothing` -kaavalla kuin 019:ssä, `account_key='juha'`. Avaa Kalenteri-näkymä, tarkista että Juhan tapahtuma näkyy agendassa suoraan (EI hyväksyntäjonossa, ks. "Kalenterin periaate" -osio) täsmälleen kuten Katrinkin syötteet, "uusi"-tagilla jos tekijää ei tunnisteta Katriksi. **Tärkein uusi tarkistus:** jos jaettu perhekalenteri näkyy MOLEMMILLA tileillä (yksi rivi `account_key='katri'`, toinen `account_key='juha'`, molemmat osoittavat samaan jaettuun kalenteriin), sen tapahtumien pitää näkyä agendassa TÄSMÄLLEEN KERRAN, ei kahdesti. Jos jomman kumman tilin synkka epäonnistuu, tarkista Vercelin lokeista virheilmoitus per syöte — toisen tilin virhe ei saa estää toisen tilin syötteitä synkkautumasta.
