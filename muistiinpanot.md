...
- **2026-07-20 — UI-palaute: arkisto siirretty overlay-dialogista Laiturin sisäiseksi kokoontaitettavaksi osioksi:** Katri testasi b730b73-erää ja koki erillisen overlay-arkiston "liian pois"-tuntuiseksi — vaati arkiston asuvan Laiturin OMAN listan sisällä, kevyenä kokoontaitettavana "🗄 Arkisto (N)" -osiona, oletuksena kiinni, rivit selvästi himmennettynä (`opacity`, ei vain harmaa teksti) kun auki. Korvasi `#laituri-arkisto-overlay`-dialogin `.laituri-arkisto-osio`:lla suoraan `#laituri-list`:n alla, `paivitaLaituriArkisto()` (yksi funktio, korvaa vanhan linkki+overlay-parin) säilyttää käyttäjän auki/kiinni-tilan uudelleenpiirron yli. `index.html`/`script.js`/`style.css`, `sw.js` v77.
