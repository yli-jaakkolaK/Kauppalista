...
- `ICLOUD_USERNAME` / `ICLOUD_APP_PASSWORD` — Katrin tili. Käyttäjätunnus on Apple ID -kirjautumisosoite (**jos CalDAV-login palauttaa 401:** ensimmäinen kokeiltava korjaus on vaihtaa `@icloud.com`-muotoon), salasana appleid.apple.com:ista (Kirjautuminen ja suojaus → Sovelluskohtaiset salasanat) luotu sovelluskohtainen salasana, EI oikea iCloud-salasana. Merkitty Vercelissä sensitiiviseksi. **Jo asetettu.**
- `ICLOUD_USERNAME_JUHA` / `ICLOUD_APP_PASSWORD_JUHA` — Juhan tili, sama periaate kuin yllä. **Katrin mukaan jo asetettu Verceliin 2026-07-08 illalla** — jos synkka silti valittaa puuttuvista Juhan tunnuksista, tarkista Vercelin ympäristömuuttujista että nimet ovat TÄSMÄLLEEN nämä (helppo kirjoitusvirhepaikka, esim. `_JUHA`-pääte unohtuu).
...
- **Testattu 2026-07-08 illalla oikealla datalla** (`sql/014_kalenteri_syotteet.sql` ajettu, syöte lisätty) — tästä testistä löytyi puute joka korjattiin heti: kaikki perheen tapahtumat eivät olleet jaetussa kalenterissa, osa eli Juhan henkilökohtaisissa kalentereissa joita Katrin tunnukset eivät nähneet. Korjattu lisäämällä tuki toiselle CalDAV-tilille, ks. "Useampi CalDAV-tili" yllä ja `sql/017_kalenteri_tilit.sql` (AJETTAVA ennen seuraavaa testiä)
...
2. **Kalenterisyötteet** (tämä osio, yllä) — max 4 päivää aikabudjetti. ✓ Perustoiminta testattu 2026-07-08 oikealla datalla, tästä löytyi tarve toiselle CalDAV-tilille (Juhan henkilökohtaiset kalenterit) — korjattu samana iltana, `sql/017_kalenteri_tilit.sql` AJETTAVA, testaamatta vielä Juhan tilillä
...
- [x] **Ulkoisen kalenterin tuonti Satamaan** — koodi valmis ja testattu oikealla datalla 2026-07-08. `sql/014_kalenteri_syotteet.sql` AJETTU. Testissä löytyi puute (osa tapahtumista Juhan henkilökohtaisissa kalentereissa, ei näkynyt Katrin tilin kautta) — korjattu samana iltana lisäämällä tuki kahdelle CalDAV-tilille, ks. "Useampi CalDAV-tili" -kappale "Kalenterisyötteet"-osiossa. **Uusi migraatio `sql/017_kalenteri_tilit.sql` PITÄÄ ajaa** ennen kuin Juhan kalentereita voi lisätä `kalenteri_syotteet`-tauluun (`account_key='juha'`).
...
- [x] **Ulkoisen kalenterin tuonti, koko putki läpi** — testattu 2026-07-08, toimii. Vaiheet (0–4) alla säilytetty historiana/muistilistana jos joskus lisätään uusi syöte alusta asti, mutta perustoiminta on jo todistettu:
...
- [ ] **UUSI, tekemättä: Juhan CalDAV-tili (aja `sql/017_kalenteri_tilit.sql` ensin).** Lisää `kalenteri_syotteet`-tauluun rivi Juhan henkilökohtaiselle kalenterille: `tyyppi` = `icloud`, `tunniste` = TÄSMÄLLEEN Juhan kalenterin näyttönimi, `account_key` = `'juha'`, `mode` = valinnan mukaan. Avaa Kalenteri-näkymä, tarkista että Juhan tapahtuma nousee hyväksyntäjonoon täsmälleen kuten Katrinkin syötteet. **Tärkein uusi tarkistus:** jos jaettu perhekalenteri näkyy MOLEMMILLA tileillä (yksi rivi `account_key='katri'`, toinen `account_key='juha'`, molemmat osoittavat samaan jaettuun kalenteriin), sen tapahtumien pitää nousta jonoon/agendaan TÄSMÄLLEEN KERRAN, ei kahdesti — jos sama tapahtuma näkyy kahdesti, duplikaattisuoja ei toiminut oikein. Jos jomman kumman tilin synkka epäonnistuu (esim. tunnukset väärin), tarkista Vercelin lokeista virheilmoitus per syöte — toisen tilin virhe ei saa estää toisen tilin syötteitä synkkautumasta.
