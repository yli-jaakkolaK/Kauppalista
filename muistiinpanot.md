...
- [ ] **Kalenterisilta, ➕-nappi `window.open`-korjauksen jälkeen (2026-07-21) — EI VIELÄ TESTATTU oikealla laitteella PWA:ssa.** Tähän mennessä todistettu: (1) data:-URI RIKKI PWA:ssa (valkoinen sivu), korjattu api/ics.js:llä; (2) api/ics.js TODISTETTU OIKEAKSI Safarissa (✓✓ suora hyppy Applen näkymään käsin avattuna); (3) silti napista PWA:ssa ei tapahtunut mitään (`window.location.href` ei navigoi standalone-tilassa) — korjattu `window.open(url, '_blank')`:lla. Testi: napauta "➕ Lisää kalenteriin" ASENNETUSSA PWA:ssa (ei Safarissa) — pitäisi nyt avautua Applen esitäytetty tapahtumanäkymä (kuten Safarissa jo todistetusti avautuu). Jos EI vieläkään toimi PWA:sta, kerro tarkalleen mitä tapahtuu (avautuuko edes uusi välilehti/konteksti? jääkö PWA:n sisään?) — seuraava kokeiltava olisi kokonaan erillinen `<a target="_blank">`-linkkielementti `window.open()`:n sijaan, tai `rel="noopener noreferrer"`-lisäys.
...
- **2026-07-21 — Kalenterisilta, ➕-nappi korjattu KAHDESSA VAIHEESSA Katrin kahden peräkkäisen laitetestin ansiosta:** (1) Ensimmäinen testi: aikaistettu tunnistus (🕐-merkintä + nappi) toimii täydellisesti, MUTTA ➕-napin painallus avasi vain tyhjän valkoisen sivun asennetussa iOS-PWA:ssa — `data:text/calendar`-URI (etukäteen merkitty epävarmaksi) osoittautui todella rikki. Korjattu uudella `api/ics.js`-palvelinreitillä (`Content-Type: text/calendar`), `avaaKalenteriSilta()` navigoi sinne data-URIn sijaan. (2) **Katrin oma ERIYTTÄVÄ jatkotesti** (käsin Safarissa vs. napista PWA:ssa) paljasti ettei tämä vielä riittänyt: sama osoite hyppäsi ✓✓ Safarissa suoraan Applen näkymään (todisti `api/ics.js`:n täysin oikeaksi), mutta napista asennetussa PWA:ssa ei tapahtunut mitään — tarkka juurisyy: **standalone-PWA nielee `window.location.href`-navigoinnin ei-HTML-vastaukseen**, täysi Safari ei. Korjattu `window.open(url, '_blank')`:lla — pakottaa saman, juuri todistetun toimivan Safari-reitin myös PWA:sta. `.ics`-rakennuslogiikka siirretty script.js:stä api/ics.js:ään, vanhat client-puolen apufunktiot poistettu kuolleena koodina. `api/ics.js` (uusi), `script.js`, `sw.js` v81→v82. **EI VIELÄ TESTATTU oikealla laitteella PWA:ssa `window.open`-korjauksen jälkeen** — sama testi toistettava.
...
**Korjaus:** `sql/067` (`ankkurit.event_date`), `api/aly-nightly.js` (tallentaa `event_date` jokaiselle osumalle), `api/ics.js` (UUSI — .ics-generointi + tarjoilu oikealla Content-Type:llä), `script.js` (`avaaKalenteriSilta()`: navigoi palvelinreitille `window.open(url, '_blank')`:lla, vanhat `icsEscape()`/`muotoileIcsAika()`/`rakennaIcsTapahtuma()`-funktiot poistettu kuolleena koodina), uusi nappi `loadAnchorCandidates()`:ssä ja `piirraHetkiSiltaKortti()`:ssä (molemmat kutsuvat samaa `avaaKalenteriSilta()`:ia). `sw.js` v82. **TESTATTU: aikaistettu tunnistus+merkintä TOIMII oikealla laitteella. `api/ics.js` TODISTETTU OIKEAKSI Safarissa (✓✓ suora hyppy Applen näkymään). `window.open`-korjaus (PWA:n standalone-navigointiongelma) EI VIELÄ TESTATTU oikealla laitteella** — sama testi toistettava asennetussa PWA:ssa: ➕ → pitäisi avautua Applen esitäytetty tapahtumanäkymä (kuten Safarissa jo todistetusti avautuu).
