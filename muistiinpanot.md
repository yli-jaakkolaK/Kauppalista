...
- [ ] **UUSI, tekemättä: Juhan tilin nimikorjauksen jälkitesti (aja `sql/031_kalenteri_juha_nimikorjaus.sql`, ks. PALUU.md OSA E) — EI TESTATTU.** Tavoite: `GET /api/caldav-sync` näyttää 8 syötettä, 0 virhettä. Tarkista että jaettu "Yhteinen kalenteri" (Juhan tilin nimi samalle kalenterille kuin Katrin "Perhekalenteri") näkyy agendassa TÄSMÄLLEEN KERRAN, ei kahdesti — todistaa UID-duplikaattisuojan toimivan myös kahden tilin välillä. Juhan oman "Oma"-kalenterin tapahtumien (todennäköisesti ne perjantain "kadonneet" menot) pitäisi ilmestyä agendaan "uusi"-tagilla. Vanhan poistetun "Katri Rantanen" -tupla-kalenterin data (jos sitä ehti tulla) pitäisi hävitä itsestään peilisäännöllä.
