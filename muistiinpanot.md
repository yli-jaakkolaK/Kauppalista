...
│   ├── 020_tyhjenna_kalenteri_odottavat.sql — kertaluonteinen siivous: BUGI A/B -korjausta edeltäneet 15 virheellistä jonoriviä pois (osittain vanhentunut, ks. 022)
...
**Jonon tyhjennys (`sql/020_tyhjenna_kalenteri_odottavat.sql`):** olemassa olevat 15 riviä syntyivät ennen näitä korjauksia ja ovat osin virheellisiä/vaillinaisia. Katri ei ollut hyväksynyt/hylännyt yhtäkään, niin koko `kalenteri_odottavat`-taulu tyhjennettiin turvallisesti. **HUOM: tämä migraatio on nyt osittain vanhentunut** — `kalenteri_odottavat` ei enää ole käytössä ollenkaan (ks. "Kalenterin periaate" -osio), niin sen tyhjentäminen on harmitonta mutta tarpeetonta. Uusi, kattavampi siivous on `sql/022_kalenteri_puhdas_alku.sql`, ks. Ajolista #2 alla.
...
2. **`sql/022_kalenteri_puhdas_alku.sql`** — poistaa KAIKKI aiemmin synkatut rivit (`kalenteri_tapahtumat` where `syote_id is not null`) ja tyhjentää `kalenteri_odottavat`-taulun kokonaan. EI KOSKE käsin lisättyihin tapahtumiin. **Turvallinen ajaa uudelleen** (tyhjän datan poistaminen on no-op). Tarkoitus: puhdas pöytä uudelle mallille — vanhan mallin (hyväksyntäjono) aikana syntynyt data ei ole enää relevanttia.
