...
- [ ] **Kalenterisilta, ➕-napin uusi palvelinreitti (2026-07-21) — EI VIELÄ TESTATTU oikealla laitteella.** Ensimmäinen tekniikka (data:-URI) testattiin ja todettiin RIKKI oikealla asennetulla iOS-PWA:lla (valkoinen tyhjä sivu). Korjattu `api/ics.js`-palvelinreitillä. Testi sama kuin ennen: odota (tai luo) hetki-tunnistus jolla on selkeä aika+aihe, napauta "➕ Lisää kalenteriin" OIKEALLA iPhonella — tarkista avautuuko iOS:n oma "uusi tapahtuma" -näkymä esitäytettynä oikealla otsikolla/päivällä/ajalla. Jos EI vieläkään avaudu odotetusti, kerro tarkalleen mitä tapahtuu (raaka teksti? lataus? edelleen tyhjä sivu?) — seuraava kokeiltava tekniikka olisi `Content-Disposition`-otsikon säätö tai `webcal:`-skeema.
...
- **2026-07-21 — Kalenterisilta, ➕-nappi korjattu oikean laitteen testin jälkeen:** Katrin testi vahvisti kalenterisillan aikaistetun tunnistuksen (🕐-merkintä + nappi) toimivan täydellisesti, MUTTA ➕-napin painallus avasi vain tyhjän valkoisen sivun asennetussa iOS-PWA:ssa — `data:text/calendar`-URI-tekniikka (etukäteen merkitty epävarmaksi) osoittautui todella rikki olevaksi. Korjattu Katrin suosittelemalla, iOS:ssä perinteisesti luotettavimmalla tavalla: uusi `api/ics.js`-palvelinreitti palauttaa `.ics`-sisällön oikealla `Content-Type: text/calendar` -otsikolla, `avaaKalenteriSilta()` navigoi sinne data-URIn rakentamisen sijaan. `.ics`-rakennuslogiikka siirretty script.js:stä api/ics.js:ään, vanhat client-puolen apufunktiot poistettu kuolleena koodina. `api/ics.js` (uusi), `script.js`, `sw.js` v81. Testattu Node-skriptillä (oikea sisältö/otsikko, virheet käsitellään). **EI VIELÄ TESTATTU oikealla laitteella — ➕-napin uusi käytös vaatii saman testin uudelleen.**
...
**KORJAUS (2026-07-21):** `data:`-URI korvattu palvelinreitillä — uusi `api/ics.js`-endpoint palauttaa saman `.ics`-sisällön oikealla `Content-Type: text/calendar` -otsikolla, ja `avaaKalenteriSilta()` navigoi sinne (`window.location.href`) datauren rakentamisen sijaan. Tämä on perinteisesti luotettavin reitti iOS Safarin/PWA:n natiiviin `.ics`-käsittelyyn, koska WebKit tunnistaa MIME-tyypin HTTP-vastauksen otsikosta eikä `data:`-skeeman sisällöstä. `.ics`-rakennuslogiikka (`icsEscape()`, `muotoileIcsAika()`, `rakennaIcsTapahtuma()`) siirretty script.js:stä `api/ics.js`:ään sellaisenaan (sama sisältö, vain toimituspaikka vaihtui) — endpoint ei tarvitse tietokantayhteyttä eikä autentikointia, se on puhdas muotoilufunktio jolle annetaan `otsikko`/`pvm`/`aika` query-parametreina (script.js tuntee ne jo valmiiksi). Testattu Node-skriptillä: kelvollinen syöte tuottaa oikean `.ics`-rungon oikealla Content-Type-otsikolla, virheellinen/puuttuva syöte palauttaa selkeän 400-virheen.

**Korjaus:** `sql/067` (`ankkurit.event_date`), `api/aly-nightly.js` (tallentaa `event_date` jokaiselle osumalle), `api/ics.js` (UUSI — .ics-generointi + tarjoilu oikealla Content-Type:llä), `script.js` (`avaaKalenteriSilta()` navigoi nyt palvelinreitille, vanhat `icsEscape()`/`muotoileIcsAika()`/`rakennaIcsTapahtuma()`-funktiot poistettu kuolleena koodina), uusi nappi `loadAnchorCandidates()`:ssä ja `piirraHetkiSiltaKortti()`:ssä (molemmat kutsuvat samaa `avaaKalenteriSilta()`:ia). `sw.js` v81. **TESTATTU: aikaistettu tunnistus+merkintä TOIMII oikealla laitteella (vahvistettu 2026-07-21). ➕-napin uusi palvelinreitti EI VIELÄ TESTATTU oikealla laitteella** — Node-testi vahvistaa vain että endpoint palauttaa oikean sisällön/otsikon, ei sitä miten iOS lopulta käsittelee sen. Sama testi toistettava: ➕ → pitäisi avautua Applen esitäytetty tapahtumanäkymä.
