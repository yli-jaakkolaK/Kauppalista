...
**✓ Hytti-yksityisyys toiselta käyttäjältä — TESTATTU JA VAHVISTETTU (2026-07-17).** Juhan painava linjaus (kalenteriyksityisyys ei kriittinen, mutta Hytin sisältö EI SAA KOSKAAN näkyä toiselle) todennettu molempiin suuntiin — "Testipäivä to 16.7." -listan OSA C läpäisty. Ei enää avoin riski.
...
**Iso ristiriitapaketti RAKENNETTU 2026-07-17** — ks. "Ristiriitapaketti" -osio. Perustuu OLEMASSA OLEVAAN päällekkäisyysmerkkiin muuttamatta sen ydinlogiikkaa, mutta KUMOAA aiemman "eri henkilöt = ei koskaan merkkiä" -säännön (nyt juuri PÄINVASTOIN: eri henkilöt = punainen "kuka hoitaa?"). **EI VIELÄ TESTATTU kahden käyttäjän kanssa** — ks. PALUU.md OSA X, `sql/059`–`060` ajettava ensin.
...
**E3-keskiporras V1 "Äly toimii, ihminen valvoo" VALMIS ja pushattu** keskiviikkoiltaan mennessä (ks. "E3-keskiporras V1" -osio) — kolme uutta migraatiota (`sql/041`–`043`), uusi yöajo-endpoint (`api/aly-nightly.js`), ankkuriehdotukset etusivulle, "Mitä äly on tehnyt" -loki Asetuksiin. **Ensimmäinen oikea yöajo 16.7. paljasti uudelleennostosilmukka-bugin, korjattu samana päivänä** kahdella lisämigraatiolla (`sql/044`–`045`, "Hetki vs. ikkuna" -luokittelu) + kotivalikon uusi pallura — ks. "Bugikorjaus: E3-yöajon uudelleennostosilmukka + Hetki vs. ikkuna" -osio. AJA `sql/041`–`045` ennen torstain testejä. **EI TESTATTU vielä** — testataan torstaina VIIMEISENÄ (ks. PALUU.md OSA O), vasta kun VAIHE 2:n perusosat on ensin kuitattu. Ei koskettu odottavaan pakettiin (ristiriitalippu, ankkurilähetys toiselle).

**RIVIEN UI-REMONTTI VALMIS** (`sql/046`, ei muita migraatioita) — pitkät rivit eivät enää katkea liian aikaisin ("tyyny…"), Varasto lukee/rivittää täydellä leveydellä, elävät listat (Muistilaput/Kauppalista/Kalenteri/Hytti) saivat kompaktimman ⋯-valikon harvemmin tarvituille toiminnoille — ks. "Bugikorjaus: Rivien UI-remontti" ja "Design-periaate: Varastossa lista nukkuu" -osiot. AJA `sql/046` ennen PALUU.md:n OSA P -testiä. **EI TESTATTU vielä oikealla laitteella.**
