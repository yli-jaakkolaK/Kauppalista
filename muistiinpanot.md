...
- [ ] **UUSI, tekemättä: Juhan tilin nimikorjaus + Oma-kalenterin Hytti-scope (aja järjestyksessä `sql/031` → `sql/033` → `sql/032`, ks. PALUU.md OSA E) — EI TESTATTU.** 033 lisää Juhan rivin `hytti_omistajat`-tauluun automaattisesti (sähköpostihaku, ei UUID:n käsinkopiointia) — AJETTAVA ennen 032:ta. Tavoite: `GET /api/caldav-sync` näyttää 8 syötettä, 0 virhettä. Tarkista että jaettu "Yhteinen kalenteri" (Juhan tilin nimi samalle kalenterille kuin Katrin "Perhekalenteri") näkyy agendassa TÄSMÄLLEEN KERRAN, ei kahdesti — todistaa UID-duplikaattisuojan toimivan myös kahden tilin välillä. **Juhan oman "Oma"-kalenterin tapahtumat EIVÄT enää näy perheen agendassa** (scope='hytti' 032:n jälkeen) — sen sijaan Juha lisää testimenon "Oma"-kalenteriinsa, tarkista että se näkyy HÄNEN Hytissään/kalenterissaan mutta EI NÄY Katrin missään näkymässä (scope-symmetria, sama malli kuin Katrin opiskelusyötteillä). Vanhan poistetun "Katri Rantanen" -tupla-kalenterin data (jos sitä ehti tulla) pitäisi hävitä itsestään peilisäännöllä.
