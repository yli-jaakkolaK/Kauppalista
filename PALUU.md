...
8. `sql/032_juha_oma_hytti_scope.sql` — Juhan "Oma"-kalenteri Hytin scopeen (ks. OSA E) — **VAATII** että `hytti_omistajat`-taulussa on jo rivi `henkilo='juha'` -> Juhan oikea auth-tunniste (ks. kohta 6 alla, "Tärkeä käsin täytettävä kohta migraation 027 jälkeen") — TARKISTA TÄMÄ Table Editorista ennen ajoa, muuten Juha ei näkisi omaa kalenteriaan edes omassa Hytissään
...
Ennen migraatiota: tarkista `?listaa=juha` (ks. yllä kohta A.3, rivi 6) että kalenterien nimet täsmäävät. Aja `sql/030_kalenteri_syotteet_data_juha.sql`, `sql/031_kalenteri_juha_nimikorjaus.sql`, JA `sql/032_juha_oma_hytti_scope.sql` (031 korjaa nimiongelman joka löytyi ensimmäisestä synkkayrityksestä, 032 tekee Juhan omasta kalenterista Hytti-scopen — ks. muistiinpanot.md "Kalenterisyötteet"- ja "Hytti v1..."-osiot). **Ennen 032:ta:** tarkista Table Editorista että `hytti_omistajat`-taulussa on rivi `henkilo='juha'` Juhan oikealla auth-tunnisteella (ks. kohta 6 alla) — muuten Juha ei näkisi omaa kalenteriaan omassa Hytissäänkään.
